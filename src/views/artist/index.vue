<template>
  <div class="artist">
    ...
    <div class="intro">
      ...
      <div class="control" v-if="!isShowAllIntro" @click=" isShowAllIntro = true">完整歌手介绍<i class="fa fa-angle-down"></i></div>
      <div class="control" v-else @click=" isShowAllIntro = false">收起<i class="fa fa-angle-up"></i></div>
    </div>
    <div class="bg"></div>
    <div class="hotSongs">
      ...
      <ul class="hotSong-list">
        <li v-for="(item,index) in artistHotSongsData" :key="item.id">
          <div class="rank" :style="{ color:'#999999'}">
          ...
          <router-link :to="'/song/'+ item.id">
            <div class="content">
              <div class="text">
                ...
                <div class="singer">{{ item.ar[0] != null ?item.ar[0].name : ''}} {{artistAlbumData.ar[1] != null ? '/' + ' ' + artistAlbumData.ar[1].name : ''}} - {{item.al.name}}</div>
              </div>
            </div>
    ...
    <div class="album">
      <router-link :to="'/album/'+ artistAlbumData.al.id">
        ...
        <div class="detail">
          ...
          <div class="content">
            <div class="name">
              {{ artistAlbumData.al != null ? artistAlbumData.al.name : ''}}
            </div>
            <div class="singer">{{artistAlbumData.ar[0] != null ? artistAlbumData.ar[0].name : ''}} {{artistAlbumData.ar[1] != null ? artistAlbumData.ar[1].name : ''}}</div>
    ...
    getArtist(){//歌手接口
      ...
      getArtist({
        ...
        if(data.code == 200){
          ...
          this.artistHotSongsData = data.hotSongs.filter((item,index)=> index < 5);
        }
      })
    },
    getArtistAlbum(){//歌手专辑接口
      let id = this.$route.params.id;
      getArtistAlbum({
        params:{
          id,
