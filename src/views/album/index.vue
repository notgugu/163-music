  ...
  <div class="album">
    <div class="album-header" :style="`background-image: url(${albumDetailData.picUrl})`">
      ...
      <div class="text">
        ...
        <div class="artist">歌手：<span>{{albumDetailData.artists != null ? albumDetailData.artists[0].name : ''}}</span></div>
        <div class="date">{{publishTime}}</div>
      </div>
    </div>
    <div class="album-intro" v-if="!isShowAllIntro">
      简介：{{albumDetailData.description}}
      <i class="fa fa-angle-down" @click="isShowAllIntro = !isShowAllIntro"></i>
    </div>
    <div class="album-introAll" v-else>
      简介：{{albumDetailData.description}}
      <i class="fa fa-angle-up" @click="isShowAllIntro = !isShowAllIntro"></i>
    </div>
    ...
    getAlbumComment(offset = 0){//获取评论
      ...
      getAlbumComment({
        ...
        if(data.code == 200){
          ...
          this.albumNewCommentData = this.albumNewCommentData.concat(data.comments);
        }
      })
    },
    getDate(){//处理时间
...
.album{
  ...
  .album-header{
    ...
    position: relative;
    height: 372px;
    background: rgba(0,0,0,0.5) 50% 50%;
